...
import { ClientApiOptions, Constructor, InferredResult, ServiceOperation, ServiceProperties } from './workers/aws-sdk';

type PoolOptions = ConstructorParameters<typeof Piscina>[0];

class WorkerPool extends Piscina {
    #drained = true;
    #done = false;
...
/**
 * Class to manage the pool of threads where we will have workers
 * to interact with the AWS APIs by using its SDK.
 */
export class WorkerPoolAwsSdk extends WorkerPool {
    constructor(poolOptions?: PoolOptions) {
        super({
    ...
    >(params?: ClientApiOptions<S, C, O, E, N>): Promise<InferredResult<S, C, O, E, N>> {
        if (this.done) {
            throw Error('Not allowed to make an API call after the worker pool has been flagged as done.');
        }
        this.begin();
        try {
